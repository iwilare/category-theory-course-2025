...
1. *(Existence of evaluation)*. There is a functor $\textsf{eval} : C \times [C,D] \to D$, defined like this: on objects $\textsf{eval}(X,F) := F(X)$ we send the pair of an object $X$ of $C$ and a functor $F : C \Rightarrow D$ to the object of $D$ that $F$ sends $X$ to. Exercise: check the rest of the definition.
...
there is an isomorphism $$\texttt{Arr}_D(F(-),=) \cong \texttt{Arr}_C(-,G(=))$$ in the functor category $[C^\textsf{op}\times D, \text{Prog}]$, where arrows are natural transformations. This takes care of the naturality requirement as well as the requirement that this isomorphism is there for every single $X$ and $Y$.
