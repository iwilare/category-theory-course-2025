...
fn append<A>(l1: List<A>, l2: List<A>) -> List<A> {
    match l1 {
        Empty => l2,
        Cons(a, tail) => Element(a, append(tail, l2)),
    }
}
...
fn add_end<A>(l: List<A>, v: A) -> List<A> {
    match l {
        Empty => oneElement(v),
        Cons(a, tail) => Element(a, add_end(tail, v)),
    }
}
...
fn join<A>(lists: List<List<A>>) -> List<A> {
    match lists {
        Empty => Empty,
        Cons(list, tail) => append(list, join(tail)),
    }
}
...
a **natural transformation** is given by the following choices:
...
- I can do this definition in a single fell swoop by defining a single function in Rust *because* of the `<A>` notation. But in theory, nothing theoretically stops me to define a family of *separate* functions, one of each specific type, like this:
...
*Equationally in Rust* the following two functions are equivalent program:
...
- Case `xs = Empty`:
  ...
  ```
    Empty.List_map_f().head()
  = Empty.head()
  = None
  ```
  ```
    Empty.head().Maybe_map_f()
  = None.Maybe_map_f()
  = None
  ```
- Case `xs = Element(a, as)`:
  ...
  ```
    Element(a, as).List_map_f().head()
  = Element(f(a), List_map_f(as)).head()
  = Some(f(a))
  ```
  ```
    Element(a, as).head().Maybe_map_f()
  = Some(a).Maybe_map_f()
  = Some(f(a))
  ```
...
- Case `xs = Empty`:
  ...
  ```
    Empty.List_map_f().length()
  = Empty.length()
  = 0
  ```
  ```
    Empty.length().ConstInt_map_f()
  = Empty.length()
  = 0
  ```
- Case `xs = Element(a, as)`:
  ...
  ```
    Element(a, as).List_map_f().length()
  = Element(f(a), as.List_map_f()).length()
  = 1 + as.List_map_f()
  ```
  ```
    Element(a, as).length().ConstInt_map_f()
  = 1 + length(as)
  ```
...
```
Pair(xs, a).add_end_pair().List_map_f() = Pair(xs, a).PairListA_map_f.add_end_pair()
```
Or, in other words,
```
Pair(xs, a).add_end_pair().List_map_f() = Pair(xs.List_map_f(), f(a)).add_end_pair()
```
...
- Case `xs = Empty`:
  ...
  ```rust
    Empty.List_map_f().reverse()
  = Empty.reverse()
  = Empty
  ```
  ```rust
    Empty.reverse().List_map_f()
  = Empty.List_map_f()
  = Empty
  ```
- Case `xs = Element(a, as)`:
  ...
  ```rust
    Element(a, as).List_map_f().reverse()
  = Element(f(a), as.List_map_f()).reverse()
  = (as.List_map_f().reverse()).add_end(f(a))
  = as.List_map_f().reverse().add_end(f(a))
  ```

  ```rust
    Element(a, as).reverse().List_map_f()
  = as.reverse().add_end(a).List_map_f() // Naturality of add_end<A>
  = as.reverse().List_map_f().add_end(f(a))
  ```
...
*(Claim.)* Every parametric function definable in a programming language which *CANNOT* distinguish on the specific type `X` that you're defining the function on defines a natural transformation (as long as you can talk about the functors involved and you have checked that these are indeed functors.)
